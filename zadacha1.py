...
#Moving the ghost
#Move up (GORE)
def move_up(ghost_state,obstacles,board_size):
    ghost_y = ghost_state[1]
    ghost_x = ghost_state[0]
    count=0
    while(ghost_y+1< board_size and [ghost_x,ghost_y+1] not in obstacles and count+1<=3):
        ghost_y+=1
        count+=1
    if([ghost_x,ghost_y+1]in obstacles and count==1 and [ghost_x,ghost_y+2] not in obstacles and ghost_y+2 <board_size ):
        ghost_y+=2
    if([ghost_x,ghost_y+2]in obstacles and count==0 and [ghost_x,ghost_y+3] not in obstacles and ghost_y+3 <board_size):
        ghost_y+=3
        
    return ghost_y
#Move right (DESNO)
def move_right(ghost_state,obstacles,board_size):
    ghost_y = ghost_state[1]
    ghost_x = ghost_state[0]
    count=0
    while(ghost_x+1<board_size and [ghost_x +1 , ghost_y] not in obstacles and count+1 <=3):
        ghost_x+=1
        count+=1
    if([ghost_x+1,ghost_y]in obstacles and count==1 and [ghost_x+2,ghost_y] not in obstacles and ghost_x+2 <board_size ):
        ghost_x+=2
    if([ghost_x+2,ghost_y]in obstacles and count==0 and [ghost_x+3,ghost_y] not in obstacles and ghost_x+3 <board_size):
        ghost_x+=3
    return ghost_x
class GhostOnSkates(Problem):
    ...
    def goal_test(self, state):
        ...
        return state == (self.n-1,self.n-1)

    @staticmethod
    def check_valid(state, walls, n):
        if(state not in walls and state[0]<n and state[1]<n):
            return True
        return False
    ...
    def successor(self, state):
        ...
        for key in actions.keys():
            new_ghost = (state[0]+actions[key][0],state[1]+actions[key][1])
            if(self.check_valid(new_ghost,self.walls,self.n)):
                successors[key] =new_ghost
    ...
    def h(self, node):
        ...
        #if the ghost is in the same row as the pacman
        if(node.state[1]==self.goal[1]):
            value+=1
        #if the ghost is in the same column as the pacman
        if(node.state[0]==self.goal[0]):
            value+=1
        
        return (abs(node.state[0]-self.n-1) + abs(node.state[1]-self.n-1))/3
...
if __name__ == '__main__':
    ...
    print(astar_search(problem,problem.h).solution())
