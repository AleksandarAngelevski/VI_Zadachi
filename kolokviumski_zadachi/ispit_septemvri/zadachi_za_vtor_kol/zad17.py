...
def transform_dataset(data):
    ...
    return [ [(row[0]+row[-2])] + row[1:len(row)-2] + [row[-1]] for row in data]


def acc(y_preds, y):
    count = 0
    for yp, yt in zip(y_preds, y):
        if yp == yt:
            count += 1 
    print(f"Broj na tochnite predviduvanja: {count} od vkupno {len(y_preds)}")
...
if __name__ == '__main__':
    C = int(input())
    percent = int(input())
    train, test =split_dataset(per=percent, data=transform_dataset(dataset),c=C)
    ...
    train2, test2 = split_dataset(per=percent, data=transform_dataset(dataset), c=C)

    train2_x = [row[:-1] for row in train2]
    train2_y = [row[-1] for row in train2]

    test2_x = [ row[:-1] for row in test2]
    test2_y = [row[-1] for row in test2]

    train2_x, test2_x = scale_data(train2_x, test2_x)

    y_preds = nb.predict(test_x)
    print(f"Tochnost so zbir na koloni: {acc(y_preds, test_y)}")

    nb2.fit(train2_x, train2_y)
    y_preds2 = nb2.predict(test2_x)
    print(f"Tochnost so skalirani koloni: {acc(y_preds2, test2_y)}")
