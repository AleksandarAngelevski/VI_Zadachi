...
def split_dataset(data, P):
    dataP = [ x for x in data if x[-1] == 1]
    dataN = [ x for x in data if x[-1] == 0]
    train = dataP[:int(len(dataP) * P / 100)] + dataN[:int(len(dataN) * P / 100)]
    test = dataP[int(len(dataP) * P / 100):] + dataN[int(len(dataN) * P / 100):]
    return train, test
...
if __name__ == '__main__':
    ...
    clsf["Random Forest"] = rf
    nn = MLPClassifier(hidden_layer_sizes=(10,), activation='relu', learning_rate_init=0.001, random_state=0)
    clsf["MLP"] = nn
    ...
    for name, model in clsf.items():
        ...
        accuracy = acc(test_y, pred)
        
        if max == "":
            max = name
        else:
            if clsf_acc[max] < accuracy:
                max = name
        clsf_acc[name] = accuracy
    ...
    print(f"Najgolema tocnost ima klasifikatorot {max}")
