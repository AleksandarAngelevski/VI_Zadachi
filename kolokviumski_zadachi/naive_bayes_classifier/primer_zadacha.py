...
def modify_data(dataset): 
    ...
    return [[row[0]+row[-2]] + [el for inx,el in enumerate(row) if inx not in [0,10]]  for row in dataset]
...
def split_dataset(C,P,dataset):
    ...
    if C==0:
        good_p = int(P/100 * len(good))
        bad_p = int(P/100 * len(bad))
        train = good[:good_p] + bad[:bad_p]
        test = good[good_p:] + bad[bad_p:]
    else:
        good_p = int((100-P)/100 * len(good))
        bad_p = int((100-P)/100 * len(bad))
        train = good[good_p:] + bad[bad_p:]
        test = good[:good_p] + bad[:bad_p]
...
def main():
    ...
    train_x, test_x, train_y, test_y = split_dataset(C,P,modify_data(dataset))

    train_x_scaled, test_x_scaled  = scale_dataset(train_x,test_x)
    
    nb_classifier_1 = GaussianNB()
    nb_classifier_1.fit(train_x, train_y)

    nb_classifier_2 = GaussianNB()
    nb_classifier_2.fit(train_x_scaled, train_y)

    print("Broj na podatoci vo train se:", len(train_x))
    print("Broj na podatoci vo test se:", len(test_x))

    print("Tochnost so zbir na koloni:",nb_classifier_1.score(test_x,test_y))
    print("Tochnost so zbir na koloni i skaliranje:",nb_classifier_2.score(test_x_scaled,test_y))
