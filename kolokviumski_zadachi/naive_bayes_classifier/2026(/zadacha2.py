...
if __name__ == '__main__':
    ...
    dataset = [[float(a) for a in e] for e in dataset]
    percent = int(len(dataset)*.85)
 
    classifier = GaussianNB()

    train_x = [e[:-1] for e in dataset[:percent]]
    train_y = [e[-1] for e in dataset[:percent]]
    test_x = [e[:-1] for e in dataset[percent:]]
    test_y = [e[-1] for e in dataset[percent:]]
    ...
    input = [ float(e) for e in input().split(" ")]
    input_pred = classifier.predict(input)[0]
    input_preds = classifier.predict_proba(input) 
