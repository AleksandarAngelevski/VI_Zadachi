...
slicing_index = int( len(dataset) * 0.85)


X = [row[:-1] for row in dataset]
Y = [row[-1] for row in dataset]
X = [row[:col_idx] + row[col_idx+1:] for row in X]
train_x = X[:slicing_index]
test_x = X[slicing_index:]
train_y = Y[:slicing_index]
test_y = Y[slicing_index:]

classifier = RandomForestClassifier(random_state=0, criterion=criterion, n_estimators=num_trees)
...
record_formatted = [int(e) for e in record.split(" ")]
record_formatted = record_formatted[:col_idx]+record_formatted[col_idx+1:]
print(f"Accuracy: {acc}")
print(classifier.predict([record_formatted])[0])
print(classifier.predict_proba([ record_formatted])[0])
