    ...
    def successor(self, state):
        ...
        for i in range(len(state)):
            for j in range(len(state)):
                new_state = tuple(self.color(state,(j,i)))
                succ["x: "+str(i)+", y: "+str(j)] = tuple(new_state)
    ...
    def goal_test(self, state):
        for i in state:
            if 0 in i:
                return False
        return True
    def color(self, state,position):
        cpy = list([list(a) for a in state])
        sign = 1 if cpy[position[0]][position[1]]==0 else 0
        if(position[0]>0): #COLOR_UP 
            cpy[position[0]-1][position[1]] = 1 if cpy[position[0]-1][position[1]]==0 else 0
        if(position[1]+1<self.size ): #COLOR_RIGHT
            cpy[position[0]][position[1]+1] = 1 if cpy[position[0]][position[1]+1]==0 else 0
        if(position[0]+1<self.size): #COLOR_DOWN
                cpy[position[0]+1][position[1]] = 1 if cpy[position[0]+1][position[1]]==0 else 0
        if(position[1]>0 ): #COLOR_LEFT
                cpy[position[0]][position[1]-1] = 1 if cpy[position[0]][position[1]-1]==0 else 0
        cpy[position[0]][position[1]] =1 if cpy[position[0]][position[1]]==0 else 0
        cpy = tuple([tuple(a) for a in cpy])
...
if __name__ == "__main__":
    ...
    for i in range(size):
        for k in range(size):
            matrica[k].append(mat[j])
            j+=1
    for i in range(size):
        matrica[i] = tuple(matrica[i])
    tuple(matrica[i])
    # for i in range(size):
    #     print(matrica[i])
    problem = Checkers(matrica,size)
    # print(matrica)
