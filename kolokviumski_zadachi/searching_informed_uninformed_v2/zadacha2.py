...
def unwrap_state(state):
    return [ list() if len(list(pillar))==1 and '' in list(pillar) else list(pillar) for pillar in state]
class TowerProblem(Problem):
    def __init__(self, initial, goal=None):
        self.initial = initial
        self.goal = goal
    ...
    def successor(self, state):
        dict = {}
        list_state = unwrap_state(state)
        
        for i in range(len(list_state)):
            for j in range(len(list_state)):
                if i==j: continue
                if len(list_state[i])==0:
                    continue
                
                list_state_cpy = [ list(l) for l in list_state ]
                # print("BEFORE:")
                # print(list_state_cpy)
                if(len(list_state_cpy[j])!=0 and list_state_cpy[j][-1]<list_state_cpy[i][-1]):
                    continue
                list_state_cpy[j].append(list_state_cpy[i].pop())
                # print("AFTER:")
                # print(list_state_cpy)
                
                dict[f"MOVE TOP BLOCK FROM PILLAR {i+1} TO PILLAR {j+1}"] = build_state_from_list(list_state_cpy)
...
def build_state_from_string(line):
    t_line = tuple([tuple(pillar.split(",")) for pillar in line.strip().split(';')])
    return t_line
def build_state_from_list(line):
    t_line = tuple([tuple(pillar) for pillar in line])
    return t_line
if __name__ == "__main__":
    ...
    t_line = build_state_from_string(line)
    t_goal_line = build_state_from_list(unwrap_state(build_state_from_string(goal_line)))
    # print(t_goal_line)
    # print(t_line)
    
    problem = TowerProblem(initial=t_line, goal=t_goal_line)
