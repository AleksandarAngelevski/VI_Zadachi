...
def modify_data(dataset):
    new_dataset = [[row[0]+row[-2]] + [el for ind, el in enumerate(row) if ind not in [0,10]] for row in dataset]
    return new_dataset

def split_dataset(C,P,dataset):
    good = [row for row in dataset if row[-1]=='good']
    bad = [row for row in dataset if row[-1]=='bad']
    
    if(C==0):
        train = good[:int(P/100 * len(good))] + bad[:int(P/100 * len(bad))]
        test = good[int(P/100 * len(good)):] + bad[int(P/100 * len(bad)):]
        train_x, train_y = [row[:-1] for row in train], [row[-1] for row in train]
        test_x, test_y = [row[:-1] for row in test], [row[-1] for row in test]
    else:
        train = good[int((100-P)/100 * len(good)):] + bad[int((100-P)/100 * len(bad)):]
        test = good[:int((100-P)/100 * len(good))] + bad[:int((100-P)/100 * len(bad))]
        train_x, train_y = [row[:-1] for row in train], [row[-1] for row in train]
        test_x, test_y = [row[:-1] for row in test], [row[-1] for row in test]
...
if __name__ == '__main__':
    file_name = 'kolokviumski_zadachi/vtor_kol_podgotovka/zadacha_1_data.txt'
    ...
    new_dataset =modify_data(dataset)
    train_X, test_X, train_Y, test_Y = split_dataset(C, P,new_dataset)
    ...
    bayes_2 = GaussianNB()
    bayes_2.fit(train_X_scaled, train_Y)

    print(f'Tochnost so zbir na koloni: {bayes_1.score(test_X, test_Y)}')
    print(f'Tochnost so zbir na koloni i skaliranje: {bayes_2.score(test_X_scaled, test_Y)}')
