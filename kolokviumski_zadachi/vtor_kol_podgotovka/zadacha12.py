...
def generate_nn(neuron_size, learning_rate):
...
if __name__ == '__main__':
    ...
    features = [float(e) for e in input().split(" ")]

    nn = generate_nn(neurons, lr)
    ...
    if train_acc <= test_acc *1.15:
        ...
        print(nn.predict(features)[0])

    else:
        print("Se sluchuva overfitting")

        train_X = [ row[:column_index] + row[column_index+1:] for row in train_X]
        test_X = [ row[:column_index] +  row[column_index+1:] for row in test_X]

        nn.fit(train_X, train_y)
        features = features[0]
        features = [features[:column_index] + features[column_index:]]
