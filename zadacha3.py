    ...
    def check_valid(state):
        for x, y in state:
            if x < 0 or x > 4 or y < 0 or y > 4:
                return False
        return True
    # ((x1, y1), (x2, y2), (x3, y3), (x4, y4), (x5, y5)) state
    def get_moving_block(self,state):
        id= 0
        for i in range(5):
    ...
    def successor(self, state):
        ...
        for i in range(5):
            if(state[i] != self.goal[i]):
                flag = True
                for j in range(4):
                    new_state = [list(l) for l in state] #tuple -> list
                    
                    new_state[i] = [new_state[i][0]+ moves[j][0],new_state[i][1]+moves[j][1]] #derive the new state
                   
                    new_state = tuple([tuple(l) for l in new_state]) # list -> tuple 
                    if(self.check_valid(new_state)):
                        succ["Pomesti kvadratche "+str(i+1)+" "+directions[j]] =  new_state
            if flag:
                break
