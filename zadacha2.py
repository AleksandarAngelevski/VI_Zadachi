...
class RingProblem(Problem):
    ...
    def successor(self, state):
        actions_ = self.actions(state,1)
        succ = dict() 
        for key in actions_.keys():
            succ[key] = actions_[key]
        return succ
    # Tuple of pillars and appropriate rings
    #1st pillar 2nd pillar 3rd pillar
    # ((x,),    (y,),       (z,))
    def emptyTuples(self,topka):
    ...
    def actions(self, state,i=0):
        x = self.emptyTuples(state)
        action_to_state ={} #dictionary of the valid actions -> state mappings
        #x (1, (0,))
        list_state = [list(el) for el in state] #state tuple as a list

        
        for i in range(len(state)):
            for j in range(len(state)):
                list_tmp= [list(l) for l in list_state]
                
                if(len(list_tmp[i])==0 ):
                    list_tmp[i].append('')
                if(i!=j and list_tmp[i][-1]!=''):
                    if(list_tmp[i][-1]<=list_tmp[j][-1] or list_tmp[j][-1] ==''):
                        if(list_tmp[j][-1]==''):
                            list_tmp[j][-1] =(list_tmp[i][-1])
                            list_tmp[i].remove(list_tmp[i][-1])
                        else:
                            list_tmp[j].append(list_tmp[i][-1])
                            list_tmp[i].remove(list_tmp[i][-1])
                        
                        if(len(list_tmp[i])==0):
                           list_tmp[i].append('')
                        action_to_state["MOVE TOP BLOCK FROM PILLAR "+str(i+1)+" TO PILLAR "+str(j+1)] = tuple([tuple(l) for l in list_tmp])
        
                         	

#;6,6,4,4,2,2;;
#;;6,6,4,4,2,2;
        return action_to_state.keys() if i==0 else action_to_state

    
    
    def move_ring(self,fr0m,to,arr):
        lista = list([list(e) for e in arr])
    def result(self, state, action):
...
if __name__ == "__main__":
    ...
    for i in line.split(';'):
        ...
        for j in i.split(','):
            tempList.append(j)
        initial_state.append(tuple(tempList))
    line = input()
    goal_state = []
    for i in line.split(';'):
        tempList = []
        for j in i.split(','):
            tempList.append(j)
        goal_state.append(tuple(tempList))
    initial_state = tuple(initial_state)
    goal_state = tuple(goal_state)
    problem = RingProblem(initial_state,goal_state)
    print("Number of action "+str(len(breadth_first_graph_search(problem).solution())))
